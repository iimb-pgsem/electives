...
# $Id: p3results.cgi,v 1.1 2006/08/12 18:21:25 a14562 Exp $
...
sub main()
{
    ...
    my $sth = $dbh->prepare("SELECT rollno, request FROM changes");
    ...
    my ($rollno, $request);
    $sth->bind_columns(\$rollno, \$request);
    my %choices;

    while ($sth->fetch()) {
      $choices{$rollno} = $request;
    }
