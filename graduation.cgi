...
my $photo_path_dir="/pgsem/graduation/uploads/images";
# end global data
...
sub print_profile_form()
{
    ...
    if ($status == 0) {
        if ($rec{'photopath'}) {
            ...
            print "<img src=\"${url}cgi-bin/images.cgi/$rec{'photopath'}\" " .
                "width=\"100\">", br;
        }
    }
...
sub save_image_file ($$)
{
    ...
    my $file = "$files_dir/$photo_path_dir/$rollno$ext";

    open OUT, ">$file" or return undef; # TODO better error reporting
    ...
    return "$photo_path_dir/$rollno$ext";
}
